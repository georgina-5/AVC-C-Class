...
hardware_exchange();
	...
	while(check_red < 160){						//quadrant 2 ends at red square, 160 is just a test value
		
		while (Count < 13){						//puts values from a[0] to a[12]
			
			while (Count_X < 260){				//TEST LINE FIX LATER
				sample_array[Count_X] = (int)get_pixel(Count_Y, Count_X, 3);
				if (sample_array[Count_X] > 100){	//if light is above a certain value, 100 is just a test value
					sample_array[Count_X] = 1;		//then that pixel will be determined to be "white"
				}
				else{								//else that pixel will be determined to be "black"
					sample_array[Count_X] = 0;		
				}
				Count_X = Count_X +20;
			check_red = (int)get_pixel(Count_Y, Count_X, 0);
		
			for(int sample_number = 0; sample_number < 13; sample_number++){
